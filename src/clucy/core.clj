...
(defn- opts->field-type
  [{:keys [stored indexed analyzed norms] :as opts}]
  (doto (FieldType.)
    (.setStored (if (false? stored) false true))
    (.setIndexOptions (if (false? indexed)
                        IndexOptions/NONE
                        IndexOptions/DOCS_AND_FREQS_AND_POSITIONS_AND_OFFSETS))
    (.setTokenized (if (false? analyzed) false true))
    (.setOmitNorms (if (false? norms) true false))))

(defn- field-type->opts
  [^FieldType field-type]
  (let [indexed (not (= (.indexOptions field-type) IndexOptions/NONE))]
    {:stored   (.stored field-type)
     :indexed  indexed
     :analyzed (and indexed (.tokenized field-type))
     :norms    (and indexed (not (.omitNorms field-type)))}))

(defn- add-field
  "Add a Field to a Document.
  Following options are allowed for opts:
  ...
  ([document key value opts]
     (.add ^Document document
           (Field. (as-str key) (as-str value) (opts->field-type opts)))))

(defn- concat-values
  "Concatenate all the maps values being stored into a single string."
  [map-in]
  (apply str (interpose " " (vals map-in))))

(defn- map->document
  "Create a Document from a map."
  [map opts]
  (let [non-stored (keep (fn [[key opts]]
                           (when (false? (:stored opts)) key))
                         opts)
        document (Document.)]
    (doseq [[key value] map]
      (add-field document key value (opts key)))
    (when *content*
      (add-field document :_content (concat-values (apply dissoc map non-stored))))
    document))
         ...
         (assoc  :_fragments (highlighter m) ; so that we can highlight :_content
                 ...
                 :_opts (into {}
                              (for [^Field f (.getFields document)
                                ...
                                [(keyword (.name f)) (field-type->opts (.fieldType f))])))
         (dissoc :_content)))))
